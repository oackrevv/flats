...
layui.use(['form','jquery',"layer"],function() {
    ...
    function showNotice(){
        layer.open({
            ...
            btn: ['确定'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">无公告</p></div>',
    ...
    function lockPage(){
        ...
        var head_img = userInfo.find('img').attr('src');
        var name = userInfo.find('.adminName').text();
        layer.open({
            title : false,
            type : 1,
            content : '<div class="admin-header-lock" id="lock-box">'+
                            '<div class="admin-header-lock-img"><img src="'+head_img+'" class="userAvatar"/></div>'+
    ...
    $("body").on("click","#unlock",function(){
        ...
        if($(this).siblings(".admin-header-lock-input").val() == ''){
            ...
            $.post('/admin/common/unlock', {password: _this.siblings(".admin-header-lock-input").val()}, function (res) {
                if (res.code) {
                    layer.closeAll("page");
